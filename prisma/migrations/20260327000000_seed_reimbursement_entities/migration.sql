...
-- 5) Attach reimbursement entity to SYSTEM role (system role should have all entities)
WITH target_entity AS (
  SELECT "id"
  FROM "entities"
  WHERE "name" = 'reimbursement'
...
-- 6) Attach manage_reimbursement entity to SYSTEM role (system role should have all entities)
WITH target_entity AS (
  SELECT "id"
  FROM "entities"
  WHERE "name" = 'manage_reimbursement'
