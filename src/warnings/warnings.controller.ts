...
export class WarningsController {
  ...
  @ApiOperation({
    ...
    description:
      'Record a warning against the user. Only users with access to the user entity can create warnings.',
  })
  @ApiParam({ name: 'userId', description: 'User ID (CUID format)' })
  @ApiResponse({
    status: 201,
  ...
  @ApiResponse({ status: 400, description: 'Invalid body (e.g. empty message)' })
  @ApiResponse({ status: 404, description: 'User not found' })
  async createWarning(
    @Param('userId', CuidValidationPipe) userId: string,
  ...
  @ApiOperation({
    ...
    description:
      'Remove a warning from the user. Only users with access to the user entity can delete.',
  })
  @ApiParam({ name: 'userId', description: 'User ID (CUID format)' })
  ...
  async deleteWarning(
    ...
    return this.warningsService.delete(userId, warningId);
  }
}
